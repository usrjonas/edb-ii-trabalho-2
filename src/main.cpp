...
binarySearchTree<DataType, KeyType>::binarySearchTree Random_Fill(binarySearchTree<DataType, KeyType>::binarySearchTree tree, int times){
    srand(time(NULL));
...
template <typename DataType, typename KeyType>
binarySearchTree<DataType, KeyType>::binarySearchTree Fill(binarySearchTree<DataType, KeyType>::binarySearchTree tree, int x, int entry[x]){
    for(int i = 0; i < x; i++){
        tree.insert(entry[x]);
    }
}
//Testa se o método inserção está funcionando corretamente. Atente para o tipo de vetor que será utilizado, se será o 'values1', '2' ou '3'. Sendo esse o último parâmetro.
//Obs.: Essa função insere apenas o valor >>30<<, se atente para esse valor, em cada vetor de valores ele tem uma posição diferente.
template <typename DataType, typename KeyType>
